...
df = pd.read_csv("llama3_embeddings.csv")
df
...
fila = X.iloc[0]
primera_fila = fila.tolist()
tamano1 = len(primera_fila)
print(f"Tamaño de los vectores: {tamano1}")
# Hyperparameters
...
def ejecucion_modelo(X, y, input_size, hidden_size, num_layers, num_classes,learning_rate,batch_size,num_epochs,porcentage):
  ...
  print(f"Entrenando modelo con {num_epochs}, factor de aprendizaje {learning_rate} y porcentage de corpus de {1-porcentage}")
  ...
  ruta_guardar_modelo = f"{ruta_figura_incom}/modelo_entrenado_{num_epochs}_{learning_rate}.pth"
